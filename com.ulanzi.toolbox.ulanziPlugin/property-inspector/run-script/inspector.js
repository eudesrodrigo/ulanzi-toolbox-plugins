const PI_UUID = 'com.ulanzi.ulanzistudio.toolbox.runscript';
...
function loadSettingsToForm(settings) {
  ...
  document.getElementById('script').value = settings.script || '';
  document.getElementById('args').value = settings.args || '';
...
for (const id of ['label', 'script', 'args', 'cwd', 'terminal']) {
...
document.getElementById('browse-script').addEventListener('click', () => {
  pendingPicker = 'script';
  $UD.selectFileDialog('script(*.sh *.py *.js *.rb *.zsh *.bash)');
});
